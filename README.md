...
14. Логика, которая не меняется от проекта к проекту (авторизация, etc) должна быть обособлена в отдельный слой. Это сильно упростит всем жизнь, когда концепция в очередной раз изменится
15. Предусмотреть механизм подписок на те данные, которые изменяются в реальном времени
