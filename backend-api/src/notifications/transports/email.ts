...
export const userAddedEmailTpl = (payload: any): MailDataRequired => ({
  ...
  html: `
    ...
    Чтобы завершить регистрацию, перейдите <a href="${process.env.AYNEED_BACKEND_HOST_URL}/registerConfirm?email=${payload.to}&code=${payload.code}">перейдите по ссылке</a>.
    `,
});
